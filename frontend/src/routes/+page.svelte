...
<script lang="ts">
    ...
    function TsaveNote() {
        ...
        if (localStorage.getItem("storage-id-array") == null) {
            ...
            localStorage.setItem(
                "storage-id-array",
                localStorage.getItem("storage-id-array") + "," + String(id)
            );
        }
        ...
        if (SignedIn) {
            // upload to remove
            var url =
                "http://localhost:8081/?https://github.com/SaicharanKandukuri/test-re";
            // @ts-ignore
    ...
    async function getAccessTocken(code: any) {
        const ENDPOINT = "https://cint-proj-notes-frontend-saicharankandukuri.vercel.app/api/getAuthCode";
        const res = await Promise.resolve(
            fetch(ENDPOINT, {
                method: "POST",
                body: JSON.stringify({
...
<div class="my-auto h-screen p-12 rounted-lg flex flex-col">
        ...
        {#if !SignedIn}
            <p class="select-none antialiased text-sl dark:text-slate-50">
                ...
                <a
                    ...
                    href="https://webassembly.org/">WASM</a
                >
            </p>
        {/if}
    </div>
    ...
    <form>
        ...
        <div
            class="text-center flex items-center justify-center flex-col md:flex-row"
        >
            {#if !SignedIn}
